...
if not dados.arquivo_existe(arquivo_root):
    dados.criar_arquivo(arquivo_root)

while True:
    root = input('Nome do Usuário: ')
    senha = input('Senha do Usuário: ')
    dados.autenticacao(arquivo_root, root, senha)
    if dados.autenticacao(arquivo_root, root, senha) == True:
        interface.titulo('INICIANDO SISTEMA')
        sleep(2)
        os.system('cls')
        while True:
            # MENU Principal
